 ...
 */
var authenticate;
module.exports.authenticate = authenticate = function(username, password, domain, callback) {
  try {
 ...
 * packet. A string description of the error will be passed as the only
 * argument to the event listeners.
 */
module.exports.createServer = function (options) {
  ...
  server.on('message', function (msg, rinfo) {
    try {
      ...
      server.emit('radius-error', ex.toString());
      return;
    }

    if (packet.code != 'Access-Request') {
      server.emit('radius-error', 'Packet code error: not "Access-Request"');
      return;
    }
